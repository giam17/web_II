...
const crear_nueva_fila = (descripcion_pe, horario_pe, id_pe, sala) => {
  ...
  const contenido = `
    ...
    <td>${horario_pe}</td>
    <td>${sala ? sala.descripcion_sa : ''}</td>
    <td>
...
peliculaService.listaPeliculas()
  .then((data) => {
    data.forEach(({ descripcion_pe, horario_pe, id_pe, salas }) => {
      const nuevaLinea = crear_nueva_fila(descripcion_pe, horario_pe, id_pe, salas);
