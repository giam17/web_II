...
const crear_nueva_fila = (descripcion_bo, precio_bo, id_bo, peliculas) => {
  const fila = document.createElement('tr');
  const contenido = `
    <td class="td" data-td>${descripcion_bo}</td>
    <td>${precio_bo}</td>
    <td>${peliculas ? peliculas.descripcion_pe : ''}</td>
    <td>
...
boletoService.listaBoletos()
  .then((data) => {
    data.forEach(({ descripcion_bo, precio_bo, id_bo, peliculas }) => {
      const nuevaLinea = crear_nueva_fila(descripcion_bo, precio_bo, id_bo, peliculas);
